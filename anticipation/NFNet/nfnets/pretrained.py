...
def pretrained_nfnet(
    path, stochdepth_rate: float = 0.5, alpha: float = 0.2, activation: str = "gelu"
) -> NFNet:
    ...
    if not len(params) in layers_to_variant:
        raise RuntimeError(
            f"Cannot load file {path.absolute()}."
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Load haiku weights and convert them to .pth file."
    )
    parser.add_argument(
        "--pretrained", type=Path, help="Path to pre-trained weights in haiku format"
    )
    ...
    torch.save({"model": model.state_dict()}, str(args.pretrained.with_suffix(".pth")))
