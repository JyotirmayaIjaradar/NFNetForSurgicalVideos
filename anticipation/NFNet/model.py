...
from networks import BayesLSTMNFNet
import util
import os
import numpy as np
...
class AnticipationModel:
    def __init__(self, opts, train=True, pretrain=None):
        ...
        self.net = BayesLSTMNFNet(output_size).cuda()
        ...
        if train:
            ...
            for params in self.optimizer.param_groups:
                name = params["name"]
                if self.net.featureNet.nfnet.exclude_from_weight_decay(name):
                    params["weight_decay"] = 0
                if self.net.featureNet.nfnet.exclude_from_clipping(name):
